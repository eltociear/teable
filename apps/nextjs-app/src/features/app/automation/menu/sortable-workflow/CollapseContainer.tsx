...
const CollapseContainer = forwardRef<HTMLDivElement, ICollapseContainerProps>(
  (props, ref) => {
    const { id, children, style, handleProps, name, workflow, className, hover, isDragging } =
      props;
    ...
    return (
      <div style={{ ...style }} className={className} ref={ref}>
        ...
        >
          ...
          />
          <span className="text-sm font-semibold flex-1 text-start truncate">{name}</span>
          <div className="flex items-center">
            {!isHover && !showDropdown && !isDragging || id === 'wscb6axXWLNge6I0N' ? (
        ...
        {/* {open ? (
          <div className={classNames(open ? (isDragging ? 'hidden' : '') : 'hidden')}>
            {children}
          </div>
        ) : null} */}
        { isDragging ? null : children}
        {/* todo adpater the active status */}
        {/* {
          <div
