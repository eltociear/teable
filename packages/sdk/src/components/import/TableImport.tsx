...
interface ITableImportProps {
  ...
  onSuccess: (value: { tableId: string; viewId?: string }) => void;
}
...
export const TableImport = (props: ITableImportProps) => {
  ...
  const { children, open, onOpenChange, fileType, onSuccess } = props;
  ...
  const importTable = async () => {
    for (const [, value] of Object.entries(workSheets)) {
      ...
      if (columns.some((col) => !col.name)) {
        setErrorMessage(t('import.form.error.fieldNameEmpty'));
        return;
      }
      if (new Set(columns.map((col) => col.name.trim())).size !== columns.length) {
        setErrorMessage(t('import.form.error.uniqueFieldName'));
        return;
      }
    }
  ...
  return (
    <>
        ...
        {open && (
          <DialogContent className="flex max-h-[80%] max-w-[800px] flex-col overflow-hidden">
            <Tabs defaultValue="localFile" className="flex-1 overflow-auto">
              {step === Step.UPLOAD && (
                <TabsList>
                  <TabsTrigger value="localFile">{t('import.title.localFile')}</TabsTrigger>
                  <TabsTrigger value="url">{t('import.title.linkUrl')}</TabsTrigger>
                </TabsList>
              )}
                  ...
                  <UploadPanel
                    ...
                    onChange={(file) => {
                      ...
                      if (file && !acceptGroup.includes(file.type)) {
                        toast({ description: t('import.form.error.errorFileFormat') });
                        return;
                      }

                      if (exceedSize && file && file.size > exceedSize * 1024 * 1024) {
                        toast({
                          description: `${t('import.tips.fileExceedSizeTip')} ${exceedSize}MB`,
                        });
                        return;
                      }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('import.title.leaveTitle')}</AlertDialogTitle>
            <AlertDialogDescription>{t('import.tips.leaveTip')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('import.menu.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
                onOpenChange?.(false);
              }}
